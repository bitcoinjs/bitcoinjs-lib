...
describe('bitcoinjs-lib (transactions w/ CSV)', () => {
  ...
  function csvCheckSigOutput(_alice, _bob, sequence): Buffer {
  ...
  // Ref: https://github.com/bitcoinbook/bitcoinbook/blob/second_edition_print3_rc2/ch07.asciidoc#complex-script-example
  ...
  it(
    'can create (but fail to broadcast via 3PBP) a Transaction where ' +
      'Alice attempts to redeem before the expiry (simple CHECKSEQUENCEVERIFY)',
    // @ts-ignore
    async () => {
  ...
  it(
    'can create (and broadcast via 3PBP) a Transaction where Bob and ' +
      'Charles can send (complex CHECKSEQUENCEVERIFY)',
    // @ts-ignore
    async () => {
      // 2 blocks from now
