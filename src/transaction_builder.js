...
function extractInput(txIn, tx, vout) {
  ...
  switch (scriptType) {
    ...
    case 'multisig':
      // Strip out initial OP_0 and any missing signatures
      parsed = scriptSig.chunks.filter(function(chunk) { return chunk !== opcodes.OP_0 }).map(ECSignature.parseScriptSignature)
