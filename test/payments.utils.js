...
function tryASM(x) {
  if (Buffer.isBuffer(x)) return __1.script.toASM(x);
  return x;
}
...
function preform(x) {
  // @ts-ignore
  x = Object.assign({}, x);
  if (x.network) x.network = __1.networks[x.network];
  ...
  if (x.redeem) {
    ...
    if (x.redeem.network) x.redeem.network = __1.networks[x.redeem.network];
  }
  return x;
}
